...
class IrDataResolver:
    ...
    SELECT_BOX_ID_BY_USER_ID = "SELECT id, box_id, visible_img_file_name, unvisible_img_file_name " \
                               "from {0} where box_id % 4 = {1} and sur_t_o > 34 and (face_ret_is_check = 0 OR face_ret_is_check IS NULL)"
    ...
    def init_queue(self):
        sql = "select ir_temp_surf from  " + config.MYSQL_TABLE_NAME \
              + " WHERE is_only_collect = 0 ORDER BY create_time desc limit 1000"
    ...
    def save_matrix_2_file(self, ir_data_dto):
        ...
        v_img_array = cv2.flip(v_img_array, 1)
